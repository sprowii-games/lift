...
struct Level8 {
    ...
    void DrawUI(Assets& assets) {
        ...
        int panelW = 760;
        int panelH = 460;
        int nx = scrW / 2 - panelW / 2;
        int ny = scrH / 2 - panelH / 2;
        DrawRectangle(nx, ny, panelW, panelH, Fade(BLACK, 0.94f));
        DrawRectangleLines(nx, ny, panelW, panelH, GOLD);

        RuText(assets.font, u8"--- ЗАКЛЮЧИТЕЛЬНЫЙ ОТЧЁТ ---", nx + 34, ny + 30, 28, GOLD);
        RuText(assets.font, u8"Объект №12 вышел на крышу в 03:17.", nx + 34, ny + 92, 22, RAYWHITE);
        RuText(assets.font, u8"Халат из ячейки B-7 закрепил ложную", nx + 34, ny + 132, 22, RAYWHITE);
        RuText(assets.font, u8"личность: «сотрудник комплекса».", nx + 34, ny + 168, 22, RAYWHITE);
        RuText(assets.font, u8"Доза Z-облучения: критическая.", nx + 34, ny + 218, 22, RED);
        RuText(assets.font, u8"Объект — НЕ СОТРУДНИК. Он заблудился.", nx + 34, ny + 270, 24, RAYWHITE);
        RuText(assets.font, u8"Рюкзак — реальность. Вертолёт — симптом.", nx + 34, ny + 324, 22, YELLOW);
        RuText(assets.font, u8"[E] закрыть", nx + 330, ny + 402, 17, GRAY);
    }
