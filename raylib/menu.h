...
enum GameState { MENU, PLAYING, EXIT, CREDITS };
...
struct Menu {
    ...
    void Update(Font font) {
        ...
        Rectangle btnCredits = { (sw - bw) / 2.0f, baseY + gap * 2, bw, bh };
        if (CheckCollisionPointRec(mouse, btnPlay)) hoveredBtn = 0;
        if (CheckCollisionPointRec(mouse, btnQuit)) hoveredBtn = 1;
        if (CheckCollisionPointRec(mouse, btnCredits)) hoveredBtn = 2;
    ...
    void Draw(Font font) {
        ...
        const char* labels[] = { u8"ИГРАТЬ", u8"ВЫХОД", u8"КРЕДИТЫ(мои)" };
        float ys[] = { baseY, baseY + gap, baseY + gap * 2 };
